...
function shuffleCardImageClasses() {
  ...
  */
  ...
  let newArray = _.shuffle(imgArray);
...
function createCards(parentElement, shuffledImageClasses) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    */
    ...
    cardObjects.push(cardProperties);
  }
...
function doCardsMatch(cardObject1, cardObject2) {
  //use image class to check if they're the same
...
function incrementCounter(counterName, parentElement) {
  ...
  counters[counterName] = counters[counterName] + 1 || 1;
...
function onCardFlipped(newlyFlippedCard) {
  ...
  flipCardWhenClicked(newlyFlippedCard);
  ...
  let matches = parseInt(document.getElementById("match-count").innerHTML) || 0;

  if (matches == 6) {//all 6 pairs have been found
    //play winning audio
    winAudio.play();
  }
  else {
